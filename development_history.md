## 2026-09-21: ミニPC移設の方針（作業中）

- **状況**: 方針確定。全体Web化は後回し。コードとデータをミニPCへ移す途中
- **置き場**: コード/DB は C:、画像CSV は USB の D:\せどり総合、バックアップは NAS の Z:
- **操作**: 当面はリモートデスクトップ。Cursor はミニPCへ SSH。`.venv` はコピーせず作り直す。Docker は今は不要
- **次**: venv 再作成 → DB/config コピー → D:\せどり総合 コピー → 現場のCSV/写真受け口
