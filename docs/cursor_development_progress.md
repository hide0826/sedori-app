## 2026-09-21 ミニPC移設（方針確定・作業中）

- **状況**: 方針確定。コードの全体Web化は待たない。実行・データ・開発をミニPCへ寄せる途中
- **構成**:
  - コード・SSH開発: `C:\HIRIO\repo\sedori-app.github`
  ...
  - 当面の操作: メインPCからリモートデスクトップ。Web入り口は後から
- **開発**: Cursor はミニPCへ SSH。隣に `store-cam` 等がある。`.venv` はマシン間コピー不可（このマシンで作り直す）
- **Docker**: 価格改定の日常利用には不要。デスクトップが `python/app.py` を起動する。後回し
- **これから（Web化より先）**:
  1. ミニPCで venv 再作成 → `main.py` 起動
  2. メインPCの `python\desktop\data` と `config` をミニPCへ（直下の `data` は本番ではない）
  3. `D:\せどり総合` をミニPCの D: へ
  4. アマサーチCSVは Nextcloud 等、写真はレシート／商品の撮影画面（既存カメラ＋Tailscale）
- **やらない**: 全画面Web化完了を待ってからファイル移設、LINEを自動取込の主経路にする、同じDBを2台で開く
