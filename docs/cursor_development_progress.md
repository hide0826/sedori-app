## 2026-09-21 ミニPCで開発・運用する（方針転換）

- **状況**: 方針転換。コードの全体Web化は待たない。**このリポジトリの実装をミニPCに置き、開発も日常運用もここで行う**
- **理由**: `C:\HIRIO` の watcher / store-cam / judgment / shared など他機能の資産を、同じワークスペースで使いながらシームレスに進めるため
- **構成**:
  - コード・SSH開発・日常起動: `C:\HIRIO\repo\sedori-app.github`
  - DB・設定: `python\desktop\data` と `config`（C:。SQLiteはNASに置かない）
  - 画像・CSV本番: USB SSD の `D:\せどり総合`（フォルダ名はそのまま。パス置換しない）
  - バックアップ: NAS の `Z:`（HIRIOから直接開かない）
  - 操作: リモートデスクトップ可。正本はミニPC。メインPCで同じDBを開かない
- **今日**: Python 3.13.15 導入、`.venv` 作り直し、`main.py` / `start_hirio.bat` で起動確認（仕入DB 1422件）。FastAPI 未接続でも仕入画面は動く
- **これから**:
  1. 日常運用はミニPC（メインPCと二重書きしない）
  2. 未コピーなら `D:\せどり総合` をミニPCの D: へ
  3. 価格改定が要るときだけ FastAPI を起動
- **やらない**: 全画面Web化完了を待ってから運用、同じDBを2台で開く、メインPCの `.venv` をコピー

## 2026-09-21 ミニPC移設（旧メモ・上の節が正）

- 当初は「実行・データ・開発をミニPCへ寄せる途中」と書いていた。**いまは開発・運用ともミニPCが正**（上の節）
...
- **状況**: 完了。ブランチ `feature/sp-api` へコミット／プッシュ
- **内容**: 店舗ラベル実装後、店舗種別タグの絞り込みを廃止。評価・メモだけ残す
- **丸ピン**: ラベルOFF時は標準で青〇。評価・メモタグがある店だけその色
...
- **含む**: 白黒地図／種別タグ正式名＋自動付与／絞り込みタブ分割／サイズ保存／応答なし修正／HO・OF→ハードオフ系

## 2026-09-13 店舗種別タグ正式名称＋絞り込みタブ分割

- **名称**: BOOKOFF系／セカンドストリート系／ハードオフ系／トレジャーファクトリー系／その他
- **UI**: タグ絞り込みを「店舗種別」「評価・メモ」タブで分割
- **実装**: `store_brand_tag_service.py` / `route_map_widget.py`

## 2026-09-13 ルート地図: 分割サイズ保存＋タグ背景白

- **内容**: 地図／左パネルのスプリッタ位置を保存。タグ絞り込みは白背景
- **実装**: `route_map_widget.py`

## 2026-09-13 ブランド系タグ自動付与＋色スウォッチ

- **内容**: bookoff系／セカスト系／オフモール系／トレファク系／その他を店名自動判別で付与。タグ管理は色見本表示
- **実装**: `store_brand_tag_service.py` / `store_tags_dialog.py` 他
- **テスト**: `tests/test_store_brand_tag_service.py`

## 2026-09-13 ルート地図: 白黒モード

- **内容**: 「白黒地図」チェックで薄いグレー基調タイルに切替。緑系ルート色も見やすい色へ変更。線に白縁を付けてコントラスト向上
- **実装**: `route_map_widget.py`（既定ON）
...
- **操作**: ツールバー店舗追加は初期値が未所属。特定ルート列ヘッダ右クリックから追加するとそのルートが初期選択
- **実装**: `store_dialogs.py` / `route_kanban.py`
...
- **現状**:
  - 改定実行はルール計算＋プライスター用CSV生成まで。Amazonへの直接反映なし
  - 入口もプライスター在庫CSV依存
  - SP-APIは出品日・手数料・注文・返品の**読み取り**まで（価格 PATCH / 在庫同期は未実装）
- **方針（確定）**:
  - 「計算結果だけ PATCH」は中間案にすぎず、**真の自動化には在庫・出品データも SP-API から取得する必要がある**
  - 正しいループ: **Amazon在庫・出品取得 → 仕入DB突合 → 既存ルールで new_price → SP-APIで価格反映**
  - Amazonから取る: SKU / ASIN / 現在価格 / 在庫数 / FBA・自己発送 / 出品日 など
  - 仕入DBから取る: 仕入値・TP・価格改定ON/OFF など（Amazonにない自社データ）
  - プライスター固有の `priceTrace` は SP-API直結では基本使わない／別設計
- **実装順（修正）**: 従来ドキュメントの「価格反映→在庫同期」を逆転し、**在庫・出品同期を先（または同時）→ 価格反映**
- **候補API**:
  - 在庫・出品: Reports（出品一覧・FBA在庫など）＋ Listings GET（補助）
  - 価格反映: Listings Items `PATCH`（本命）／ Feeds（大量時）
- **参照**: `docs/sp_api_roles.md` の実装予定表を同日更新
- **次回**: 在庫・出品スナップショット取得の具体設計 → 改定タブ①ファイル選択の置き換え → 価格 PATCH
...
- **次回候補**: 在庫・出品のSP-API取得（価格改定自動化の前提）、価格改定の API 反映、財務と会計ロール申請、429時の注文取込再開
...
- **API起動/接続**（`app.py`, `main_window.py`, `client.py`）
  - FastAPIを `127.0.0.1:8000` で起動・接続し、`localhost` 接続失敗時は `127.0.0.1` にフォールバック。
  - アプリ内サーバー起動は `python/app.py` を正しいcwdで起動し、`/health` 応答確認後に接続済み表示するよう修正。
- **価格改定タイムアウト/APIブロック対策**（`client.py`, `settings_widget.py`, `routers/repricer.py`）
  - 価格改定プレビュー/実行の待ち時間を長めにし、設定タブのタイムアウト既定値・上限を拡張。
  - 価格改定処理を `asyncio.to_thread` 経由にし、重いCSV処理中もFastAPIイベントループを塞ぎにくく変更。
- **月別運用共通ロジック分離**（`utils/repricer_ladder_core.py`, `desktop/utils/repricer_ladder_core.py`）
  - PySide6依存の表UIから、30日帯・月別ルールJSON処理を非UI共通ロジックへ分離。
  - API起動時の `PySide6` import 失敗と、デスクトップ側 `utils` パス衝突による仕入DB描画エラーを解消。
- **仕入DB表示復旧**（`product_widget.py`）
  - 最新スナップショットが空でも直近の非空スナップショットから復元。
  - 異常に少ない件数でスナップショット上書きしない保険と、復旧用の「仕入DB再読込」ボタンを追加。
- **動作確認**:
  - 仕入DBスナップショット1041件を復元し、テーブル1041行表示を確認。
  - `python -m py_compile` で変更ファイルの構文確認。
- **リポジトリ**: sedori-app.github
...
- **変更**: `python/desktop/ui/antique_widget.py`
  - 相手区分コンボ・フィルタ・列グループを **法人／フリマ／個人** に整理（法人＝店舗系カラム表示用トグル1つ）。
  - **PDF** 出力では法人ルートの相手区分列・表紙の出力区分を **「法人」** に正規化。
  - 確定保存時 **支店** を `store_branch` から `counterparty_branch` へ正しく保存。
  - フィルタ帳は **ネスト QGroupBox を廃止** し縦余白を抑制。
- **変更**: `python/desktop/database/ledger_db.py`
  - `ledger_entries.counterparty_type` を **店舗／店舗法人／EC法人 → 法人** に一括更新（DB初期化時）。
- **変更**: `python/desktop/database/store_db.py`, `python/desktop/ui/store_master_widget.py`
  - **`online_stores`** に **address, phone**（マイグレーション）。電脳店舗一覧は **店舗名の直後に住所・電話**、編集ダイアログで入力。検索に住所・電話を含める。
- **Git**: （本コミット）

## 2026-05-10 メインタブ整理・DB設定のマスタ集約・フリママスタ

- **目的**: 3-6-9 を既定 UI としつつ旧タブは残して非表示にする。コード系マスタを「設定 > DB設定」に集約する。フリマは販売手数料率など将来連携用のマスタを一覧・追加編集できるようにする。
- **変更**: `python/desktop/ui/main_window.py`
  - **仕入管理**: タブ名を「旧仕入管理」に変更し **非表示**（従来の仕入＋コンディション説明サブタブは維持）。
  - **3-6-9仕入管理**: タブ名を「**仕入管理**」に変更。サブタブ「仕入データ」「コンディション説明」（後者は `ConditionTemplateWidget` を別インスタンスで表示。DB は本番 `hirio.db` と共有）。
  - **価格改定**: 従来タブを「旧価格改定」に改名し **非表示**。「3-6-9価格改定」を「**価格改定**」に改名。
- **変更**: `python/desktop/ui/store_master_widget.py`
  - 店舗マスタから **電脳プラットフォーム**・**問屋** を削除（設定側へ移設）。**電脳店舗** は店舗マスタに残置。
  - **`FleaMarketEditDialog` / `FleaMarketListWidget`** を追加。電脳プラットフォームと同レイアウトで列 **ID・コード・名称・区分（フリマ固定表示）・接頭辞・手数料率・有効・備考**。手数料率はスピンで 0〜100% または「未設定」（DB は NULL）。
- **変更**: `python/desktop/ui/settings_widget.py`
  - **DB設定** タブ内をサブタブ化: **チェーン店コード** / **電脳プラットフォーム** / **問屋** / **フリマ**。
- **変更**: `python/desktop/database/store_db.py`
  - **`flea_markets`** テーブル新規（`platform_code`, `platform_name`, `category` 既定フリマ, `code_prefix`, `fee_rate`, `is_active`, `notes`）。CRUD・一覧・コード重複チェックを実装。
- **Git**: feat(desktop): メインタブ整理・DB設定マスタ集約・フリママスタ（flea_markets）
...
### 2026-02-17（画像管理テンプレ起点フォルダ・JAN画像除外・証憑仕訳日時修正）
- **チャット**: Agentモード（実装）
- **内容**:
  1. 画像管理タブのAmazonテンプレ書き込みで「保存先デフォルト」ボタンを追加し、仕入れフォルダ配下などを起点としてテンプレ保存先を選べるように変更
  2. JANグループツリーの各グループ1枚目画像に白いチェックボックスを追加し、デフォルトでJAN画像をGCSアップロード対象から除外（チェックOFFで例外的に1枚目も送信）
  3. 証憑管理タブの確定ボタンを常時押せるようにしつつ、仕訳帳登録時の取引日付を「日付＋時刻」で保存し、その新フォーマットに合わせて重複判定とデバッグログを修正
  4. 画像管理タブのAmazonテンプレ書き込みで、保存ダイアログの初期フォルダをユーザーが設定した「保存先デフォルト」フォルダから開くように変更
- **変更ファイル**: python/desktop/ui/image_manager_widget.py, python/desktop/ui/receipt_widget.py, docs/cursor_development_progress.md
...
### 2026-03-03（古物台帳レシートURL連携・仕入健全度スコア(PRO)）
- **内容**:
-  1. **古物台帳タブとレシート画像URLの連携**
     - `ledger_db.py`:
       - `ledger_entries` テーブルに `sku` カラムを追加（スキーマ新規作成＆既存DBマイグレーション対応）。
       - `insert_ledger_rows()` で SKU を含めて保存できるように変更。
     - `antique_widget.py`:
       - 統一スキーマの共通列に `SKU` 列を追加し、閲覧・出力タブのテーブルにも SKU を表示。
       - 店舗列の「レシート番号」を「レシート画像URL」にリネームし、入力フォームのラベルも同様に変更。
       - `reload_ledger_rows()` 内で `LedgerDatabase.query_ledger()` の結果に対し、`ProductDatabase` を SKU で参照して `receipt_image_url` を取得し、空の `receipt_no` に埋める `_attach_receipt_image_urls()` を追加。
       - 閲覧・出力テーブルのレシート画像URL列を青字＋下線付きリンク風にし、ダブルクリックで既定ブラウザを開くように変更。
       - 仕入管理タブからの取込時、備考(`notes`) を仕入データのコメントから自動移行せず、古物台帳側で別管理できるように変更。
-  2. **証憑管理タブのレシート画像URLカラム名称統一**
     - `receipt_widget.py`:
       - レシート一覧テーブルヘッダの「画像URL」を「レシート画像URL」に変更。
       - ダブルクリック動作の docstring とコメントも、レシート画像URLを開くことが分かる表現に統一。
-  3. **仕入管理（開発）タブの仕入健全度スコア（PRO版限定）**
     - `inventory_widget.py`:
       - コメント列の先頭1〜2桁の数字と「除外」「テスト」有無から 3/6/9 グループを自動判定（空欄・NaN は 6 扱い）。除外・テスト行は統計から除外。
       - 統計ラベルに「仕入健全度(件数3-6-9)」を追加し、3/6/9 の件数比率と 9 グループ比率から「安全圏 / 注意圏 / 危険域」を判定（ラベル色も緑・黄・赤で変更）。
       - 「仕入健全度(金額3-6-9)」として、見込み利益を重みとした金額ベースの 3/6/9 比率と、想定見込み利益合計・実効見込み利益合計・実現率(%) を統計エリアに表示。
       - 実効見込み利益は行ごとに  
         3グループ: 見込み利益×0.9 / 6グループ: ×0.7 / 9グループ: 発送方法がFBAなら×0.6、自己発送(非FBA)なら×0.9  
         の係数を掛けて算出し、コレクター・廃盤の自己発送9グループは長期保管料ではなく送料リスクのみを反映する設計に変更。
       - 統計エリア右に「?」ボタンを追加し、3-6-9分類ルール・係数の意味・実効見込み利益の計算式（想定見込み利益合計／実効見込み利益合計／実現率）をヘルプダイアログで説明。
- **変更ファイル**:
-  - python/desktop/database/ledger_db.py
-  - python/desktop/ui/antique_widget.py
-  - python/desktop/ui/receipt_widget.py
-  - python/desktop/ui/inventory_widget.py
-  - docs/cursor_development_progress.md
...
### 2026-03-04（仕入管理（開発）3-6-9自動分類・SKUテンプレ連携）
- **内容**:
-  1. **仕入管理（開発）タブの3-6-9列追加と自動判定**
     - `inventory_widget.py`:
       - 仕入管理（開発）タブの仕入データ一覧に「3-6-9」カラムを追加（表示上は店舗コードの右に配置）。本番タブには追加しない。
       - CSV取込時にコメント列（`コメント` / `comment`）から先頭トークンを読み取り、以下のルールで各行の3-6-9コード（3P/3N/6P/6N/9P/9N）を自動判定して「3-6-9」列に入力：
         - `3` → `3P`
         - `4`, `3n` → `3N`
         - `6` → `6P`
         - `7`, `6n` → `6N`
         - `9` → `9P`
         - `10`, `9n` → `9N`
         - 空欄や上記以外（11,12…など連番を含む）→ `6P`
       - すでに「3-6-9」列に値が入っている行は自動判定の上書きを行わず、ユーザー編集を優先。
       - devモードでは store_db / route_db / condition_template_db だけ本番DBを共有し、店舗名解決やルートテンプレの参照を本番タブと揃えるように修正（仕入データやスナップショットは `data_dev/` 側を使用）。
-  2. **SKUテンプレートでの3-6-9プレースホルダ対応**
     - `sku_template.py`:
       - `SKUTemplateRenderer` に `{rule369}` プレースホルダを追加し、商品データのコメントから上記と同一ルールで 3P/3N/6P/6N/9P/9N を算出してSKU文字列に埋め込めるようにした。
       - 実装は `_get_rule369_code()` として切り出し、コメント先頭の `3/4/3n/6/7/6n/9/10/9n` を正規表現で判定し、不明・空欄は `6P` にフォールバック。
     - `config/inventory_settings.json`:
       - デフォルトのSKUテンプレートを `{date:YYYYMMDD}-{supplier}-{purchasePrice}-{rule369}-{seq:3}` に変更し、3-6-9コードをSKUに含めるように更新。
-  3. **仕入管理（開発）タブとルート登録タブの連携改善**
     - `main_window.py`:
       - ルート登録タブ（`RouteSummaryWidget`）で構築したルート情報を、本番タブだけでなく仕入管理（開発）タブ側の `InventoryWidget(dev_mode=True)` にも接続。
       - これにより、仕入管理（開発）タブでもルートテンプレ読込・照合処理・スポット仕入などのルート連携機能を本番タブと同様に利用可能にした。
...
### 2026-03-15（仕入DBのTA廃止・TP列UI・コメント補完）
- **チャット**: Agentモード（実装）
- **内容**: 仕入価格まわりを **TP1/TP2 に一本化**し、画面上の列順と表示モードを整理
- **実装完了**:
  - **`purchase_db.py`**
    - 新規作成時の `purchases` スキーマから **`ta1` / `ta2` カラム定義を削除**（今後は **TP1/TP2 相当の `tp1` / `tp2` のみ**）
    - `upsert()` の更新フィールドから **`ta1` / `ta2` を除外**（旧DBに物理カラムが残っていてもアプリからは更新しない）
    - 呼び出し側が `ta1`/`ta2` を渡したときの **`tp` への自動寄せを削除**（データは TP 側のみを正とする）
    - 既存DB向け: **`ta*` に値があり `tp*` が空のときだけ `tp*` にコピーするワンショット移行**は維持
  - **`product_widget.py`**
    - 仕入テーブルの列解決で **`商品名` の直後に `TP1` / `TP2` を挿入**（従来はステータス理由の近くにあった列を移動）
    - ツールバーに **「TP」ボタン**を追加（`画像` ボタンの近く）。押下で **TP1〜TP2 列だけ**にフォーカスする表示モード（`toggle_view_mode("tp")`）
    - 動的列展開時に **TA1/TA2（大文字小文字）を表示対象から除外**
    - コメントから TP を補完する処理で、旧 **TA* を TP* に写したあと `row` から TA* キーを `pop`** して二重管理を防ぐ
- **変更ファイル**:
  - `python/desktop/database/purchase_db.py`
  - `python/desktop/ui/product_widget.py`
  - `docs/cursor_development_progress.md`
- **次回**: 必要に応じて既存DBの `ta1`/`ta2` 列を整理する運用方針のドキュメント化
...
### 2026-03-24（3-6-9価格改定 方針確定・実装ハンドオーバー作成）
- **チャット**: Askモード→Agentモード（要件整理・ドキュメント整備）
- **内容**:
  1. `3-6-9価格改定` の運用方針を確定
  2. `akaji` 運用と Prister 側ストッパー前提を明文化
  3. 次チャットで即実装できる詳細ハンドオーバーMDを作成
- **確定した方針**:
  - SKUタグ（3/6/9）でルールを分岐
  - 改定ルールは 3/6/9 のサブタブで管理
  - Keepaは「同コンディション最安(min)」を直近値で利用
  - 毎回観測時に「ざっくり引き直し」で TP 下限へ到達させる
  - `keepa_min < TP下限` は **TP下限で固定**（自動で下げない）
  - アラートは結果テーブルの **理由列** に表示
- **追加ドキュメント**:
  - `docs/handover_3_6_9_reprice_agent.md`
    - 実装対象ファイル
    - 設定JSON拡張案
    - 価格計算ロジック
    - アラート仕様
    ...
    - 最低限の動作確認シナリオ
- **次回**:
  - 上記ハンドオーバーに沿って 3-6-9 価格改定の本実装（UI/設定/判定/計算）に着手
