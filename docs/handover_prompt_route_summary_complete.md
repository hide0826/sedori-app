...
### レシートID列のクリック・ドラッグアンドドロップ機能の実装（2025-11-19）

#### 機能概要
- **機能1:** 仕入DBタブのレシートID列をクリックすると、レシート画像ファイルを開く
- **機能2:** レシートID列のセルをドラッグアンドドロップでAmazonの中古画像入力画面などに送信可能

#### 実装内容
- **`DraggableTableWidget`クラスの作成:**
  - `QTableWidget`を継承したカスタムクラス
  - ドラッグアンドドロップ機能を実装
  - レシートID列のセルにマウスが来たときにカーソルをポインターに変更

- **`on_purchase_table_cell_clicked`メソッドの追加:**
  - レシートID列のセルをクリックしたときに呼び出される
  - レシートDBから画像ファイルパスを取得
  - `QDesktopServices.openUrl()`で画像ファイルを開く

- **`startDrag`メソッドのオーバーライド:**
  - レシートID列のセルをドラッグしたときにテキストデータとしてレシートIDを設定
  - `QMimeData.setText()`でレシートIDを設定
  - Amazonの中古画像入力画面などにドラッグアンドドロップで送信可能

- **ツールチップ機能:**
  - レシートID列のセルにツールチップを表示
  - 「クリックで画像を開く」と画像ファイルパスを表示

#### 動作
1. レシートID列のセルにマウスを移動すると、カーソルがポインター（手のアイコン）に変わる
2. レシートID列のセルをクリックすると、レシート画像ファイルが開く
3. レシートID列のセルをドラッグアンドドロップすると、レシートIDのテキストが送信される
...
## 追加作業記録（2025-02-XX 最新：画像管理機能とデータベース管理機能の拡張）

### 画像管理機能の拡張（2025-02-XX）
- **バーコード読み取りの高速化**
  - `services/image_service.py`: 画像前処理機能を追加（リサイズ・グレースケール化・コントラスト強化）
  - 大きな画像を1200px以下にリサイズして処理時間を大幅短縮
  - 画像読み取りのオプション化（skip_barcode_reading、skip_exif、skip_image_size）
  - ファイル名からJANコードを先に抽出して高速化（画像内バーコード読み取りをスキップ可能）
  - ZXingバーコード読み取りのエラー修正（rawフィールドが整数の場合に対応）
- **全画像を一覧表示**
  - バーコードが無い画像も含めて、フォルダ内の全画像を表示
  - スキャン時の高速化（EXIF・画像サイズ取得をスキップ可能）
  - 画像読み込みを非同期で実行（スキャン完了後にバックグラウンドで読み込み）
- **JAN保存時の自動グループ化**
  - JAN保存時に選択された画像の撮影日時から±4分以内の画像を自動検索
  - 該当する画像に同じJANコードを付与してグループ化
  - ツリーウィジェットを自動更新
- **SKU候補検索機能**
  - JAN保存時に仕入DBから同じJANコードを持つレコードを検索
  - SKU候補を取得してメッセージに表示（最大5件）
  - ProductWidgetへの参照を設定して仕入DBにアクセス可能に
- **フォルダ選択エリアの最小化**
  - フォルダ選択エリアの高さを最小限に設定
  - メインエリア（JANグループ・画像一覧・詳細情報）のスペースを最大化

### 商品DB・仕入DBタブに画像1〜6の欄を追加（2025-02-XX）
- **商品DBタブ**
  - テーブルに画像1〜6の列を追加（11列目〜16列目）
  - 画像パスをファイル名のみ表示（ツールチップでフルパス表示）
...
- **仕入DBタブ**
  ...
  - 画像パスをファイル名のみ表示（ツールチップでフルパス表示）
  - 画像列をクリックすると画像ファイルを開く機能
  - 商品DBから画像情報を自動付与（SKUで一致する場合）
- **データベーススキーマ**
  - `products`テーブルに`image_1`〜`image_6`カラムを追加（マイグレーション処理）
  - 既存DBへの互換性を維持

### SKU列の表示問題修正（2025-02-XX）
- **問題:** 仕入DBテーブルでSKU列のデータが表示されない
...
- 変更ファイル:
  - `python/desktop/services/image_service.py` - バーコード読み取りの高速化・ZXingエラー修正
  - `python/desktop/ui/image_manager_widget.py` - 全画像表示・JAN保存時の自動グループ化・SKU候補検索
  - `python/desktop/ui/product_widget.py` - 画像1〜6の欄追加・SKU列表示修正
  - `python/desktop/database/product_db.py` - image_1〜image_6カラム追加
  - `python/desktop/ui/main_window.py` - ImageManagerWidgetへの参照設定
  - `docs/cursor_development_progress.md` - 進捗更新
