...
```
D:\せどり総合\店舗せどり仕入リスト入れ\仕入帳\     … 同期しない（画像で大きい）
  └── YYYYMMDDルート名\
        route_template_*.xlsx     … 作成時にルート保険へもコピー
        route.json                … Web時刻・経費（Phase 1）
        商品画像\
        レシート画像\
        仕入CSV\                  … ミニPC生存時の保険。スマホの正はルート保険

D:\せどり総合\店舗せどり仕入リスト入れ\ルート保険\  … ドライブが同期するのはここだけ
  └── YYYYMMDDルート名\
        route_template_*.xlsx     … ダウン時の滞在時刻
        仕入CSV\                  … スマホからの StockList 直送先
```
...
1. Webテンプレ作成でルート箱を作る（仕入帳。画像はここだけ）
2. 店でアマサーチ CSV を共有 → **Google Drive** → `ルート保険` のその日の箱の `仕入CSV\`
3. 帰宅後「ルート箱から取込」。仕入帳の箱を選ぶと、CSV がルート保険にあればそちらを読む
...
### Drive へ載せる方法（2026-09-25 以降）

**Google ドライブのデスクトップアプリで同期するのは `ルート保険` だけ。** 仕入帳（画像で約23GB）は同期しない。

Webテンプレ作成時に、Excel と空の `仕入CSV` を次へコピーする。

`D:\せどり総合\店舗せどり仕入リスト入れ\ルート保険\YYYYMMDDルート名\`

出かける前に、このフォルダの同期完了を確認する。ミニPCが落ちたあとは、クラウドのその Excel で滞在時刻を書く。スマホの仕入CSVも、同じ箱の `仕入CSV\` に置く。帰宅後に仕入帳の箱を取り込むと、ルート保険側の新しい CSV と Excel を読む。

rclone はボタンから呼ばない。`config/rclone_route_drive.json` の `enabled` は false。

### rclone で Drive 上に箱を作成（2026-09-22〜25・停止）

当時は Google Drive デスクトップの常時同期を不要にし、テンプレ作成時だけ rclone で送っていた。以下は停止前の手順で、今は実行しない。
...
| サブフォルダ | 振り分け |
|--------------|----------|
| `仕入CSV/`（無ければ直下 `StockList_*`） | CSV取込。ネット仕入タブは「ルート箱からCSV取込」（CSVのみ） |
| `商品画像/` | 画像管理のカレントフォルダ |
| `レシート画像/` | 証憑 OCR キュー |
