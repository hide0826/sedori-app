...
| 3 | **在庫と注文の追跡** | Inventory and Order Tracking | 在庫・注文の取得 | 販売DB「SP-API更新」＋「返品・返金取込」（フラット返品レポート）。在庫同期・保管料は未実装 |
...
| 4 | **在庫・出品同期**（改定の入口。プライスターCSV代替） | 在庫と注文の追跡 + 商品の出品 |
| 5 | 価格改定の API 反映（改定の出口。Listings PATCH 等） | 料金 + 商品の出品 |
