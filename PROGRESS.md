...
**運用・機能更新の正はメインPCデスクトップ。サーバーは PWA 殻＋ダミー検証。価格改定（実行／ルール／SP-APIシミュ）と仕入（データ＋コンディション説明テンプレの薄い版）まで。次は他メニュー枠の順次載せ、または SP-API最安追従。**
...
|----------|----------|------|
| TOP | - | 殻OK |
| 価格改定 | 改定実行 / 改定ルール / SP-API改定 | 実行・ルール利用可。SP-APIはダミー取得〜プレビュー〜反映シミュ可。最安追従は準備中 |
