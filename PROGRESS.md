...
**運用・機能更新の正はメインPCデスクトップ。サーバーは PWA 殻＋ダミー検証。価格改定は改定実行／ルール／SP-API改定（ダミー取得〜プレビュー〜反映シミュ）まで。仕入はサブタブ枠あり。次はコンディション説明の薄い中身、または他メニュー枠の順次載せ。**
