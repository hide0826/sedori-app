...
**運用・機能更新の正はメインPCデスクトップ。サーバー PWA は主要メニュー薄い版まで完了。コンディション説明だけ hirio.db 接続を試験開始（API優先・localStorageフォールバック）。他メニューの本番接続は別判断。**
...
11. **他メニューの本番API/DB接続は別判断**（ルート / 商品DB など順次）
...
| 仕入管理 | 仕入データ / コンディション説明 | 仕入データ利用可。コンディション説明は **hirio.db 接続試験中**（API優先） |
