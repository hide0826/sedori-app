...
import os, json, secrets, base64, shutil, re, unicodedata, time
...
def _inspect_attach_ean13_columns(sample_rows: List[Dict], headers: List[str], *, clean: bool):
    ...
    jan_col = None
    for h in headers:
        if (h or "").strip().lower() in JAN_KEYS:
            jan_col = h
            break
    if not jan_col:
    ...
    for row in sample_rows:
        ...
        row["ean13_calc_check_digit"] = ""
        row["ean13_reason"] = ""

        if target == "":
            row["ean13_reason"] = "empty"
            continue
        if not target.isdigit():
            row["ean13_reason"] = "数字ではありません"
            continue
        if len(target) != 13:
...
def _app_version() -> str:
    # FastAPIに version を持たせていればそれ、なければ環境変数、最後は既定
    try:
        v = getattr(app, "version", None)
    except NameError:
        v = None
    return v or os.environ.get("HIRIO_APP_VERSION", "0.3.0")
...
# app.version を環境変数で上書きする
try:
    app.version = os.environ.get("HIRIO_APP_VERSION", "0.3.0")
except NameError:
    pass
# --- end add ------------------------------------------------------------------
...
def search(req: SearchReq, Authorization: Optional[str] = Header(None)):
    ...
    for rec in _iter_index():
        ...
        if not basic_ok:
            try:
                snippet_src = _read_body(Path(rec["path"])).lower()
                basic_ok = q in snippet_src
            except Exception:
                pass
        if basic_ok and match_filters(meta):
            snippet = _read_body(Path(rec["path"])).replace("\r", "")
...
class ValidateRules(BaseModel):
    ...
    empty_forbidden_columns: List[str] = Field(default_factory=list)
...
class RowValidator:
    ...
    def feed(self, rowdict: Dict[str, str], rownum: int):
        ...
        # empty は ean13 チェックの前に処理して、重複を避ける
...
def write_report_csv(report_path: Path, issues: List[Dict], newline_out: str = "CRLF"):
    ...
    # 動的にヘッダを決定（後方互換性のため）
    header = ["row", "column", "rule", "value", "message"]
    has_raw_value = any("raw_value" in it for it in issues)
    has_calc = any("calc" in it for it in issues)
    if has_raw_value:
    ...
    w.writerow(header)
    for it in issues:
        row = [it.get(h) for h in ["row", "column", "rule", "value", "message"]]
        if has_raw_value:
            row.append(it.get("raw_value"))
        if has_calc:
            row.append(it.get("calc"))
        w.writerow(row)
...
def csv_inspect(req: InspectReq, Authorization: Optional[str] = Header(None), check_ean13: bool = Query(False), clean: bool = Query(False)):
    _auth(Authorization)
    p = _csv_rel_to_abs(req.relpath)
    if not p.exists(): raise HTTPException(404, "file not found")
    data = p.read_bytes()
    enc = _csv_detect_encoding(data) if req.encoding_in == "auto" else req.encoding_in
    ...
    for i, row in enumerate(rdr):
        rows.append(row)
        if len(rows) >= max(req.sample_rows, 10) + 1: break # header+rows
...
def csv_normalize(req: NormalizeReq, Authorization: Optional[str] = Header(None), check_ean13: bool = Query(False), clean: bool = Query(False)):
    ...
    jan_cols_to_check = {col for col in out_cols if col.lower() in JAN_KEYS}
    ...
    for row in rows_iter:
        ...
        if req.drop_empty_rows and all((str(x) == "" for x in out_row_values)):
            continue
        w.writerow(out_row_values); out_count += 1
    ...
    if req.report_relpath:
        try:
            ...
            if not req.dry_run:
                ...
                by_rule = defaultdict(int)
                for it in issues: by_rule[it.get("rule")] += 1
                report_meta = {"issues": len(issues), "by_rule": dict(by_rule), "preview": issues[:20]}
        except Exception as e:
...
def csv_bulk_normalize(req: BulkNormalizeReq, Authorization: Optional[str] = Header(None)):
    ...
    for m in matches:
        ...
        try:
            ...
            issues = int(((res or {}).get("report") or {}).get("validation") or {}).get("issues", 0)
            total_issues += issues
            items.append({
                "ok": True,
                "input": in_rel,
                "output": ((res.get("report") or {}).get("io") or {}).get("out_path", ""),
                "issues": issues,
